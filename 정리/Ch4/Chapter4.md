...
> 💡 **연결고리: 3장 1차 정리(My Takeaway)로부터의 진화**
> 우리는 3장 1차 정리에서 단순 GKE 배포 흐름을 추상화하여 `Service Spec ➔ Build ➔ Store Image ➔ Deploy ➔ Validate ➔ Report`라는 공통 배포 패턴을 이끌어냈습니다. 또한, 에이전트가 EC2와 같은 가상 머신에서 강력한 권한을 쥐고 인프라를 다룰 때 생기는 보안 위협을 체감하며, **"Agentic AI의 가드레일은 프롬프트 튜닝이 아니라 엄격한 시스템 권한 경계를 기반으로 설계해야 한다"**는 핵심적 교훈을 얻었습니다.
> 
> 본 추가 실험은 바로 그 문제의식의 연장선에서 기획된 후속 설계입니다. 클라우드 중립적인 단일 명세 파일(ServiceSpec YAML)과 자연어 명령으로 멀티클라우드 배포를 처리하는 AI 에이전트 `Kiro`의 아키텍처를 제시하고, 에이전트의 위험성을 통제하기 위한 시스템적 가드레일 방안을 함께 기록합니다.
...
### 5. 시스템적 가드레일 및 권한 경계 설계
이전 해커톤에서 에이전트를 EC2 환경에 올려 직접 인프라 제어권을 쥐여주었을 때 겪었던 보안적 위험을 보완하기 위해, 본 아키텍처는 프롬프트 튜닝(지침)에 의존하는 보안이 아닌 **구조적/시스템적 격리 가드레일**을 강력하게 주입합니다.

1. **간접 제어 (Declarative IaC Generator)**
   * 에이전트(`Kiro`)가 대상 클러스터에 직접 `kubectl apply` 나 `gcloud`, `aws` CLI 명령을 쉘에서 직접 실행할 권한을 갖지 않도록 막습니다.
   * 에이전트의 역할은 오직 GitOps 저장소에 선언형 매니페스트(YAML)나 Terraform 코드만을 생성하여 `Git Commit & Push` 하는 것으로 제한됩니다.
   
2. **풀 기반 배포 (Pull-based Deploy via GitOps)**
   * 에이전트가 코드를 푸시하면 배포의 실제 수행은 클러스터 내부의 **Argo CD** 혹은 **GitOps 컨트롤러**가 수행합니다.
   * 이로 인해 에이전트는 인프라 자원의 생성/수정 크레덴셜을 직접 획득하지 않게 되어, 에이전트 권한 탈취 시에도 클러스터 외부 인프라의 직접 파괴를 방지합니다.

3. **CI 단계의 정적 가드레일 검증 (Static Policies)**
   * 에이전트가 생성한 YAML은 GitOps 저장소 PR(Pull Request) 시 자동으로 실행되는 OPA(Open Policy Agent)나 정적 분석 도구(`Conftest`, `tfsec` 등)를 통해 강제 검사됩니다.
   * `replicas` 제한 초과 방지, 허용되지 않은 포트 노출 금지, 리소스 요청/제한(Requests/Limits) 설정 누락 방지 등의 보안 가드레일을 통과해야만 머지 및 배포가 일어납니다.
   
4. **인간 승인 루프 (Human-in-the-Loop)**
   * 최종 배포 전 Git PR 단계에서 사람(플랫폼 엔지니어)의 코드 리뷰 및 최종 승인 단계를 결합하여, 중요한 운영 환경(Production)으로의 최종 반영은 반드시 사람의 동의를 얻어 진행하도록 안전장치를 마련합니다.
