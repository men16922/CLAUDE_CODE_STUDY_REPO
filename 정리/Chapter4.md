...
4장 전체 정리가 **완료**된 상태입니다.

| 구간  | 제목                             | 상태 |
| --- | ------------------------------ | -- |
| 4.1 | 관측 가능성이란                       | 완료 |
| 4.2 | 메트릭 모니터링: Prometheus + Grafana | 완료 |
| 4.3 | 로그 수집: Loki + Fluent Bit       | 완료 |
| 4.4 | 알림 설정: PrometheusRule          | 완료 |
| 4.5 | 마무리: 메모리에 작업 컨텍스트 기록           | 완료 |
| 4.6 | 4장 가드레일 살펴보기                   | 완료 |
...
3장에서는 GitOps와 CI/CD 파이프라인을 완성했습니다. 코드를 푸시하면 GitHub Actions가 이미지를 빌드하고, ArgoCD가 클러스터에 배포합니다.
...
4장에서는 클러스터에 관측 가능성 스택을 한 번에 구축합니다.
...
실습 환경은 `e2-medium` 노드 2개 정도의 작은 구성이므로 기본값보다 리소스 요청을 줄입니다.
...
Prometheus가 데이터를 제대로 수집하는지 API로 확인합니다.
...
Prometheus는 PromQL로 메트릭을 조회합니다.
...
curl -s 'http://localhost:3100/loki/api/v1/query_range' \
  ...
  --data-urlencode 'limit=3' | jq '.data.result[0].values[:3]'
```
...
1. GitOps 일관성
   `k8s/monitoring/` 디렉터리에 YAML로 저장하면 ArgoCD가 자동 관리할 수 있습니다.
...
GitOps와 ArgoCD를 사용하는 흐름에서는 알림도 코드로 남기는 편이 자연스럽습니다.
...
curl -s http://localhost:9090/api/v1/rules \
  | jq '.data.groups[] | select(.name=="notiflex-alerts") | .rules[].name'
```
...
│   ├── Alertmanager
│   ├── Loki + Fluent Bit x 2
│   └── kube-state-metrics + node-exporter x 2
├── argocd namespace
│   └── ArgoCD
└── notiflex namespace
    └── Deployment: notiflex-api v0.1.1
```
...
| 유형      | 예시                                                | 저장 방식   |
| ------- | ------------------------------------------------- | ------- |
| 결정 기록   | ArgoCD를 GitOps 도구로 선택한 이유, Loki를 선택한 이유           | 5장에서 다룸 |
...
이제 서비스가 단순히 배포되는 수준을 넘어, 상태를 관측할 수 있는 구조가 되었습니다.

* Pod와 Node 상태를 Prometheus가 수집합니다.
* Grafana에서 클러스터 상태를 대시보드로 확인합니다.
* Fluent Bit가 각 노드의 컨테이너 로그를 Loki로 보냅니다.
* Grafana에서 메트릭과 로그를 함께 볼 수 있습니다.
* PrometheusRule이 이상 조건을 감지하고 Alertmanager로 전달합니다.

---

## GitOps 관점

관측 가능성 설정도 코드로 관리합니다.
...
이제 운영 설정도 Git에 남고, PR 리뷰와 변경 추적이 가능해집니다.
