...
Active Service
  ↓
Argo Rollouts
  ├─ Blue ReplicaSet: 현재 운영 버전
  └─ Green ReplicaSet: 새 버전 Preview
```
...
> GKE Gateway L7 로드밸런서가 정상적으로 작동하며, 외부 IP를 통해 `/version` API에 접근해 `v0.1.1` 버전을 반환하는 것을 확인할 수 있습니다.
    ...
    ```bash
    kubectl port-forward svc/notiflex-api-preview -n notiflex 8080:80
...
### 📸 실습 인증 (독자 직접 캡처 필요): Argo Rollouts CLI 모니터링 화면
> [Argo Rollouts CLI 캡처 가이드]
> 터미널에서 다음 명령어를 실행하여 롤아웃이 Healthy 상태로 전환되는 화면을 직접 캡처해 보세요.
> ```bash
> kubectl argo rollouts get rollout notiflex-api -n notiflex
> ```
