...
그림 6-1. 상태·시크릿·배포 위험을 Pod 밖의 관리 계층으로 분리한 아키텍처

![그림 6-1. 엔터프라이즈 기반 정비 전체 구조](./images/ch06/fig-6-01-enterprise-foundation-overview.png)

> 그림 6-1. Gateway API 뒤의 배포 제어, Valkey 공유 상태, Secret Manager 시크릿, 관측 계층을 하나의 구조로 보여준다.
...
![그림 6-2. Pod별 인메모리와 Valkey 공유 상태 비교](./images/ch06/fig-6-02-valkey-shared-state.png)

> 그림 6-2. Pod별 로컬 카운터는 중복되지만, Valkey `INCR`은 모든 replica가 하나의 순서를 공유하게 한다.
...
### 📸 실습 인증: Valkey 공유 ID 응답

![Valkey INCR로 생성된 Notiflex ID 응답](./images/ch06/valkey-id-response.png)

> [!TIP]
> `/id` 응답의 `generated_by`는 요청을 처리한 Pod를, `id`는 Valkey에서 원자적으로 증가한 공유 번호를 보여준다.
...
![그림 6-3. Secret Manager에서 Pod까지의 시크릿 흐름](./images/ch06/fig-6-03-secret-manager-csi-flow.png)

> 그림 6-3. 시크릿 원본은 Secret Manager에 두고, Pod에는 CSI 볼륨의 읽기 전용 파일만 제공한다.
...
### 실습 검증 결과 (Secret Mount 및 API 호출 연동)

Notiflex API 파드는 보안을 위해 `scratch` 베이스 이미지로 빌드되었으므로 쉘(`sh`, `cat`) 명령어가 존재하지 않아 직접 파드 내부 파일을 엿보는 위협으로부터 보호됩니다. 대신 `kubectl describe` 및 API 정상 응답을 통해 Google Secret Manager와의 연동을 검증할 수 있습니다.

**1. 파드 내 Secret Volume Mount 및 환경변수 확인**
```bash
kubectl --context gke-sysnet4admin_book_gitaiops describe pod <pod-name> -n notiflex
```
```
Environment:
  VALKEY_ADDR:           valkey-primary.notiflex.svc.cluster.local:6379
...
Mounts:
  /mnt/secrets from secrets (ro)
```
*GKE Secret Manager CSI Driver에 의해 GCP Secret Manager의 `valkey-password` 값이 Pod 내부의 `/mnt/secrets/valkey-password` 경로에 읽기 전용(ro) 파일로 안전하게 마운트되고, 환경변수로 지정되었음을 나타냅니다.*

**2. `/id` 엔드포인트를 통한 인증 및 동작 최종 검증**
실제 API 가 마운트된 비밀번호를 올바르게 로드하여 Valkey 백엔드에 로그인 후 분산 카운터를 정상 작동시키고 있음을 API 응답 결과로 확인합니다.
```bash
curl http://$GATEWAY_IP/id
```
```json
{"id":1,"generated_by":"notiflex-api-59d4694f58-xxxx"}
```
...
![그림 6-4. Canary 점진 배포와 중단 흐름](./images/ch06/fig-6-04-canary-progressive-delivery.png)

> 그림 6-4. Canary는 트래픽 비율을 단계적으로 높이고, 문제가 보이면 `abort`로 Stable에 즉시 복귀한다.
...
### 📸 실습 인증: Argo CD Canary 토폴로지

![Argo CD에서 확인한 Notiflex Canary 리소스 토폴로지](./images/ch06/argocd-topology.png)

> [!TIP]
> Argo CD에서 `notiflex-api` Stable Service, `notiflex-api-preview` Canary Service, Rollout과 두 ReplicaSet이 하나의 GitOps 토폴로지로 연결된 상태를 확인한다.
