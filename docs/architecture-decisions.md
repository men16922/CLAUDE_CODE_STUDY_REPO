...
## ADR-008: Pod 간 공유 상태를 위한 Valkey 도입 (6장)
**시점**: 2026-07 / **결정**: Pod 간의 독립된 인메모리 상태를 중앙에서 동기화하고 중복 없는 원자적 ID 생성을 수행하기 위해 Valkey standalone 캐시를 도입합니다.
**이유**:
- Redis 프로토콜과 완전 호환되어 기존 애플리케이션 코드를 고치지 않고 사용할 수 있습니다.
- 원자적 ID 증가(`INCR` 명령어)를 통해 다중 Pod 스케일 아웃 상태에서의 ID 중복 발생 문제를 완전히 해결합니다.
- Redis의 라이선스 제약(SSPL/RSAL) 우려 없이 상용 환경에서도 안전하게 사용이 가능합니다.
- **트레이드오프**: Pod당 10~50m CPU를 사용하는 standalone 형태이므로 SPOF(Single Point of Failure) 제약이 생기나, 실습 리소스 한계 내에서 데이터 공유 기능을 구현하기에 최선의 선택입니다.

## ADR-009: 시크릿 관리를 위해 Google Secret Manager 및 CSI Driver 연동 (6장)
**시점**: 2026-07 / **결정**: GKE 내부의 Base64 암호화 수준의 Secret 한계를 넘어서기 위해, Google Secret Manager와 GKE CSI Driver, Workload Identity를 연동하여 파일 마운트 기반의 보안 구조를 수립합니다.
**이유**:
- 시크릿 값을 Git 저장소에 커밋하지 않고 구글 클라우드의 공인된 저장소(Secret Manager)에서 단일 진실 공급원(SSOT)으로 안전하게 버전 관리합니다.
- Workload Identity를 바인딩하여 Pod에 GCP SA 키 파일을 마운트하지 않는 무키(keyless) 구조를 달성해 키 탈취 위험성을 없앱니다.
- Cloud Audit Logs를 통해 비밀 값에 대한 접근 제어 이력을 정밀하게 감사(Audit)할 수 있습니다.

## ADR-010: 무중단 배포 전략을 Blue/Green에서 Canary로 전면 전환 (6장)
**시점**: 2026-07 / **결정**: 배포 시의 순간 자원 부담 완화 및 안전한 점진적 사용자 통제를 달성하기 위해 배포 전략을 Canary(20% → 50% → 80% → 100%)로 전환합니다.
**이유**:
- Blue/Green 배포 시의 순간 리소스 2배 소요(Active 2 + Preview 2) 한계를 극복하고, 작은 노드 예산 내에서 무중단 롤아웃을 안정적으로 수립합니다.
- 20%, 50%, 80% 가중치 단계별로 트래픽을 점진 노출시켜 모니터링 메트릭을 관측하고 에러 발생 시의 잠재 장애 영향 범위를 최소화합니다.
